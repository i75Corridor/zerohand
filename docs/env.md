...
| Variable | Default | Description |
|----------|---------|-------------|
| `DATA_DIR` | `.data` (repo root) | Root directory for all runtime state — Postgres data, blueprints, skills, agent sessions. |
| `SKILLS_DIR` | `$DATA_DIR/skills` | Where skill folders (`SKILL.md` + `scripts/`) are stored. Each subdirectory is one skill. |
| `BLUEPRINTS_DIR` | `$DATA_DIR/blueprints` | Where cloned blueprint repos are stored after `blueprints install`. |
...
| Variable | Description |
|----------|-------------|
| `GITHUB_TOKEN` | GitHub personal access token. Used by `blueprints install` to clone private repos and by `blueprints discover` to raise the GitHub API rate limit. **Not used for publishing** — see the `gh` CLI note below. |

### GitHub auth for publishing

**Publishing blueprints** (via the UI's "Publish to GitHub" button or `POST /api/blueprints/publish`) uses the `gh` CLI, not `GITHUB_TOKEN`. The server calls `gh repo create` as a subprocess, relying on whatever credentials `gh` has stored locally.
...
- `SKILLS_DIR` and `BLUEPRINTS_DIR` both live inside `DATA_DIR` by default, keeping all runtime state in one place that's easy to back up or wipe.
