...
export default function ChatPanel({ stepRunId, onSend }: ChatPanelProps) {
  ...
  return (
    <div className="border border-pawn-surface-700/60 rounded-xl bg-pawn-surface-900/50 p-3 space-y-2">
      ...
      <div className="flex gap-2">
        <input
          ref={inputRef}
          className="flex-1 bg-pawn-surface-800 border border-pawn-surface-700 rounded-xl px-3 py-1.5 text-sm text-white placeholder-pawn-surface-600 focus:outline-none focus:border-pawn-gold-500 focus:ring-1 focus:ring-pawn-gold-500"
        ...
        />
        <button
          className="px-2.5 py-1.5 bg-pawn-gold-600 hover:bg-pawn-gold-500 text-white rounded-xl transition-colors disabled:opacity-40"
          onClick={send}
          disabled={!input.trim()}
          title="Send"
