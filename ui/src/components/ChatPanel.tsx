...
export default function ChatPanel({ stepRunId, onSend }: ChatPanelProps) {
  ...
  return (
    <div className="border border-slate-700/60 rounded-xl bg-slate-900/50 p-3 space-y-2">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        <span className="font-medium text-slate-400">Steer agent</span>
        ...
        <div className="space-y-1 max-h-24 overflow-y-auto">
          {history.map((h, i) => (
            <div key={i} className="text-xs text-slate-500 flex gap-2">
              <span className={h.action === "steer" ? "text-amber-400" : "text-sky-400"}>
                [{h.action}]
              </span>
      ...
      <div className="flex gap-2">
        <input
          ref={inputRef}
          className="flex-1 bg-slate-800 border border-slate-700 rounded-xl px-3 py-1.5 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
